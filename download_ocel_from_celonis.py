...
url = input("insert the URL of connection to the Celonis instance -> ")
api = input("insert the API token -> ")

celonis = get_celonis(url, api)

data_pool = celonis.pools.find(input("Insert the name of the data pool to consider for the download -> "))
data_model = data_pool.datamodels.find(input("Insert the name of the data model to consider for the download -> "))
analysis = celonis.analyses.find(input("Insert the name of the analysis to consider for the download -> "))
...
for tab in tables:
    ...
    print("downloading event log for process configuration: "+tab)
...
for tab in tables:
    ...
    for tab2 in tables:
        if tab != tab2:
            ...
            print("download relations between table "+tab+" and table "+tab2+" (using the interleaved miner)")
